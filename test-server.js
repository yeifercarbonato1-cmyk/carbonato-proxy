...
const server = http.createServer((req, res) => {
  ...
  res.setHeader('Content-Security-Policy', "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; connect-src 'self'; img-src 'self' data: https:; frame-ancestors 'none'; base-uri 'self'; form-action 'self'");
  ...
  // Modelo17 admin page
  if (path.startsWith('/api/modelo17')) {
    const m17Handler = wrapHandler(require('./api/modelo17.js'));
    return m17Handler(req, res);
  }

  // Modelo18 admin page
  if (path.startsWith('/api/modelo18')) {
    const m18Handler = wrapHandler(require('./api/modelo18.js'));
    return m18Handler(req, res);
  }

  // Modelo19 admin page
  if (path.startsWith('/api/modelo19')) {
    const m19Handler = wrapHandler(require('./api/modelo19.js'));
