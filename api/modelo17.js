// api/modelo17.js — Página de administración para modelo17 Cavernícola
// GET  /api/modelo17          → página HTML (requiere auth)
// POST /api/modelo17/knowledge → actualiza conocimiento
// POST /api/modelo17/config    → actualiza configuración
// GET  /api/modelo17/test?q=   → prueba RAG query
...
<title>CAVERNÍCOLA — modelo17</title>
<style>
...
<h1>🦴 CAVERNÍCOLA</h1>
<div class="sub">modelo17 — Configuración y base de conocimiento</div>
...
<div class="section">
  ...
  <div class="row">
    <div class="field">
      ...
      <select id="mode">${['rag','full','tool'].map(m => `<option value="${m}"${config.modelo17?.knowledge?.mode === m ? ' selected' : ''}>${m.toUpperCase()} — ${m === 'rag' ? 'Inyección inteligente' : m === 'full' ? 'System prompt completo' : 'Endpoint externo'}</option>`).join('')}</select>
    </div>
    <div class="field">
      <label>Nivel Caveman</label>
      <select id="cavemanLevel">${['lite','full','ultra'].map(l => `<option value="${l}"${config.modelo17?.caveman?.default_level === l ? ' selected' : ''}>${l.toUpperCase()} — ${l === 'lite' ? 'Ligero' : l === 'full' ? 'Cavernícola' : 'Telegráfico'}</option>`).join('')}</select>
    </div>
    <div class="field">
      <label>Límite RAG</label>
      <input type="number" id="ragLimit" min="1" max="10" value="${config.modelo17?.knowledge?.rag_limit || 3}">
    </div>
  </div>
...
async function saveConfig(){
  ...
  const r=await fetch('/api/modelo17/config',{
...
async function saveKnowledge(){
  ...
  if(raw.style.display==='block'){
    ...
    text='# Base de Conocimiento — modelo17\\n\\n'+buildKnowledgeText();
  }
  const r=await fetch('/api/modelo17/knowledge',{
    method:'POST',headers:{'Content-Type':'application/json'},
...
async function testQuery(){
  ...
  const r=await fetch('/api/modelo17/test?q='+encodeURIComponent(q));
  const d=await r.json();
  if(d.found){
...
module.exports = async (req, res) => {
  ...
  // GET /api/modelo17 → página HTML
  if (pathname === '/api/modelo17' && method === 'GET') {
    if (!cookieOk(req)) {
      res.statusCode = 302;
  ...
  // POST /api/modelo17/knowledge → actualizar conocimiento
  if (pathname === '/api/modelo17/knowledge' && method === 'POST') {
    ...
    if (token) {
      try {
        ...
        await fetch(apiUrl, {
          ...
          body: JSON.stringify({
            message: `Update knowledge - ${new Date().toISOString().slice(0,10)}`,
            content: Buffer.from(text).toString('base64'),
            sha
          })
        });
      } catch(e) { console.log('[modelo17] Error GitHub knowledge:', e.message); }
    }

    return res.status(200).json({ ok: true, sections: db.sections });
  }

  // POST /api/modelo17/config → actualizar configuración
  if (pathname === '/api/modelo17/config' && method === 'POST') {
    if (!cookieOk(req)) return res.status(401).json({ error: 'Auth required' });
    let body = {};
    try {
      const chunks = []; for await (const chunk of req) chunks.push(chunk);
    ...
    // Actualizar modelo17
    if (!config.modelo17) config.modelo17 = {};
    if (!config.modelo17.knowledge) config.modelo17.knowledge = {};
    config.modelo17.knowledge.mode = mode;
    config.modelo17.knowledge.rag_limit = parseInt(ragLimit) || 3;
    config.modelo17.knowledge.enabled = true;
    config.modelo17.knowledge.tool_endpoint = '/api/knowledge';
    config.modelo17.knowledge.available_modes = ['rag', 'tool', 'full'];
    if (!config.modelo17.caveman) config.modelo17.caveman = {};
    config.modelo17.caveman.default_level = cavemanLevel;
    config.modelo17.caveman.enabled = true;
    config.modelo17.caveman.levels = ['lite', 'full', 'ultra'];
      ...
      try {
        ...
        if (getR.ok) {
          ...
          await fetch(apiUrl, {
            ...
            headers: { 'Authorization': `token ${token}`, 'Accept': 'application/vnd.github.v3+json', 'Content-Type': 'application/json' },
            body: JSON.stringify({
              message: `Update modelo17 config - ${new Date().toISOString().slice(0,10)}`,
              content: Buffer.from(JSON.stringify(config, null, 2)).toString('base64'),
              sha
            })
          });
        }
      } catch(e) { console.log('[modelo17] Error GitHub config:', e.message); }
    }
