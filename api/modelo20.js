// api/modelo20.js — Página de administración para modelo20
// GET  /api/modelo20          → página HTML (requiere auth)
// POST /api/modelo20/knowledge → actualiza conocimiento
// POST /api/modelo20/config    → actualiza configuración
// GET  /api/modelo20/test?q=   → prueba RAG query
...
<title>modelo20 — Configuración</title>
<style>
...
<div class="section">
  ...
  <div class="row">
    <div class="field">
      ...
      <select id="mode">${['rag','full','tool'].map(m => `<option value="${m}"${config.modelo20?.knowledge?.mode === m ? ' selected' : ''}>${m.toUpperCase()} — ${m === 'rag' ? 'Inyección inteligente' : m === 'full' ? 'System prompt completo' : 'Endpoint externo'}</option>`).join('')}</select>
    </div>
    <div class="field">
      <label>Nivel Caveman</label>
      <select id="cavemanLevel">${['lite','full','ultra'].map(l => `<option value="${l}"${config.modelo20?.caveman?.default_level === l ? ' selected' : ''}>${l.toUpperCase()} — ${l === 'lite' ? 'Ligero' : l === 'full' ? 'Cavernícola' : 'Telegráfico'}</option>`).join('')}</select>
    </div>
    <div class="field">
      <label>Límite RAG</label>
      <input type="number" id="ragLimit" min="1" max="10" value="${config.modelo20?.knowledge?.rag_limit || 3}">
    </div>
  </div>
...
<div class="section">
  ...
  <div class="row">
    <div class="field">
      <input type="text" id="testQuery" placeholder="Consulta de prueba..." onkeydown="if(event.key==='Enter')testRag()">
    </div>
    <button onclick="testRag()">⟫ BUSCAR</button>
  </div>
...
async function saveConfig(){
  ...
  const r=await fetch('/api/modelo20/config',{
...
async function saveKnowledge(){
  ...
  if(raw.style.display==='block'){
    ...
    text='# Base de Conocimiento — modelo20\\n\\n'+buildKnowledgeText();
  }
  const r=await fetch('/api/modelo20/knowledge',{
    method:'POST',headers:{'Content-Type':'application/json'},
...
async function testRag(){
  ...
  const r=await fetch('/api/modelo20/test?q='+encodeURIComponent(q));
  const d=await r.json();
  if(d.found){
...
module.exports = async (req, res) => {
  ...
  // GET /api/modelo20 → página HTML
  if (pathname === '/api/modelo20' && method === 'GET') {
    if (!cookieOk(req)) {
      res.statusCode = 302;
  ...
  // GET /api/modelo20/test?q=... → probar RAG
  if (pathname === '/api/modelo20/test' && method === 'GET') {
  ...
  // POST /api/modelo20/knowledge → actualizar conocimiento
  if (pathname === '/api/modelo20/knowledge' && method === 'POST') {
    if (!cookieOk(req)) return res.status(401).json({ error: 'Auth required' });
    let body = {};
    try {
    ...
    try { fs.writeFileSync(KNOWLEDGE_BASE_PATH, text, 'utf8'); } catch(e) {}
    ...
    if (token) {
      try {
        ...
        let sha = '';
        if (getR.ok) { const fileData = await getR.json(); sha = fileData.sha || ''; }
        await fetch(apiUrl, {
          method: 'PUT',
          headers: { 'Authorization': `token ${token}`, 'Accept': 'application/vnd.github.v3+json', 'Content-Type': 'application/json' },
          body: JSON.stringify({
            message: `Update knowledge (modelo20) - ${new Date().toISOString().slice(0,10)}`,
            content: Buffer.from(text).toString('base64'),
            sha
          })
        });
      } catch(e) { console.log('[modelo20] Error GitHub knowledge:', e.message); }
    }

    return res.status(200).json({ ok: true, sections: db.sections });
  }

  // POST /api/modelo20/config → actualizar configuración
  if (pathname === '/api/modelo20/config' && method === 'POST') {
    if (!cookieOk(req)) return res.status(401).json({ error: 'Auth required' });
    let body = {};
    try {
      const chunks = []; for await (const chunk of req) chunks.push(chunk);
    ...
    if (!config.modelo20) config.modelo20 = {};
    if (!config.modelo20.knowledge) config.modelo20.knowledge = {};
    config.modelo20.knowledge.mode = mode;
    config.modelo20.knowledge.rag_limit = parseInt(ragLimit) || 3;
    config.modelo20.knowledge.enabled = true;
    config.modelo20.knowledge.tool_endpoint = '/api/knowledge';
    config.modelo20.knowledge.available_modes = ['rag', 'tool', 'full'];
    if (!config.modelo20.caveman) config.modelo20.caveman = {};
    config.modelo20.caveman.default_level = cavemanLevel;
    config.modelo20.caveman.enabled = true;
    config.modelo20.caveman.levels = ['lite', 'full', 'ultra'];
      ...
      try {
        ...
        if (getR.ok) {
          ...
          await fetch(apiUrl, {
            ...
            headers: { 'Authorization': `token ${token}`, 'Accept': 'application/vnd.github.v3+json', 'Content-Type': 'application/json' },
            body: JSON.stringify({
              message: `Update modelo20 config - ${new Date().toISOString().slice(0,10)}`,
              content: Buffer.from(JSON.stringify(config, null, 2)).toString('base64'),
              sha
            })
          });
        }
      } catch(e) { console.log('[modelo20] Error GitHub config:', e.message); }
    }
